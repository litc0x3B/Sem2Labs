...
public:
    ...
    void Add(T item) override {tree.Add(item);}
    void Remove(T item) override {tree.Remove(item);}
    bool Search(T item) const override {return tree.Search(item);}
    bool Search(T item, T &foundItem) const override {return tree.Search(item, foundItem);}
